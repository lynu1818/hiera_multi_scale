# https://github.com/lucasjinreal/yolov7_d2/blob/main/demo_lazyconfig.py
...
class DefaultPredictor:
    ...
    def __init__(self, cfg):
        ...
        self.aug = T.FixedSizeCrop(crop_size=(1024, 1024))
...
def get_parser():
    ...
    parser.add_argument(
        "--output-suffix",
        help="suffix to add to output images",
        default='',
        type=str
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.input:
        if os.path.isdir(args.input):
            ...
            res = predictor(img)
            res = vis_res_fast(res, img, metadata, colors)
            filename = '_'.join(['res', Path(args.input).stem, args.output_suffix])
            cv2.imwrite(f'{filename}.jpg', res)
            # cv2.imshow('frame', res)
            #cv2.imshow('frame', res)
            #cv2.waitKey(0)
    elif args.webcam:
